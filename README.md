...
- **Надёжность персонажа**: AE4@370 (≈1.5 ч) собирает персонажа 4/4; v2 не собирает его на кровавом промпте даже на последней точке 1250 (≈6+ ч) — провал является свойством рецепта v2, а не стадии обучения;
- **Скетчи**: parity — оба несут манеру и персонажа;
- Вердикт: **турбо-рецепт (lr 2e-4, ae4, anchor 8.0) принят как production-стандарт** для style-адаптеров AnimaTrainer.
