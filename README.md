...
Режимы: `quantize_extent: below_trainable` (блоки строго ниже нижнего обучаемого — чистый конвейер признаков без риска для фиделити адаптера) | `all`. Качество: ошибка весов ~1%, детерминизм между SAC-recompute проходами бит-точный (тесты). По умолчанию выключено.
