...
### Фиксы по внешнему аудиту (2026-08)

1. **Weighting был no-op'ом** — нормировка на `weights.sum()` внутри батча отменяла bell/timestep-веса при B=1 (и в ожидании при любом B). Теперь: bell **mean-normalized** (E[w]=1 по равномерной сетке σ), лосс = `mean(w·L)` без батч-перенормировки; веса примеров валидируются при загрузке манифеста. ⚠️ Изменяет масштаб вклада σ — сравнение с траекториями до фикса осторожно.
2. **Скрытые синхронизации** — `.item()` на каждый микрошаг + `torch.any(weights<=0)` в Python-условии убраны: статистика аккумулируется GPU-тензорами, конвертация только на `log_every`; валидация весов — на этапе манифеста.
3. **`anchor_every: 4` включён во все recipe-конфиги**; в лог добавлены `anchor_loss_when_active` и `anchor_active_fraction` (усреднённый anchor_loss при каденции >1 был искусственно занижен нулями неактивных шагов).
4. **Кэш подписан контентом**: fingerprint'и (size+mtime+head/tail) VAE/TE/DiT + сигнатуры per-entry (картинка+bucket+VAE; промпт+TE+DiT) в index и metadata. Тренинг на старте жёстко валидирует (legacy-кэши — только warn). Кэш-команда грузит VAE/TE только если есть что перекодировать.
5. **Fused QKV** теперь честен с обёртками: `enabled` (иначе `lora_disabled()` не выключал QKV-LoRA — баг якоря на self_attn-scope'ах), adapter-dropout, и обходит кэш при обучаемых базах (full-weight режимы).
6. **Анализатор LoRA**: kohya-имена разворачиваются по словарю токенов (глобальный `_→.` превращал `cross_attn` в несуществующий `cross.attn` → компоненты уезжали в `other`); round-trip тест добавлен.
7. **Валидация**: детерминированный случайный выбор held-out сэмплов вместо первых N (снятие bias сортировки); baseline-бакеты соответствуют выбранным индексам.

### Матрица throughput после фиксов (`scripts/bench_throughput.py`, 1280², якорь без гейта)

| combo | ms/sample | vs baseline |
| --- | --- | --- |
| mbs1 / w0 / exact-loss | 15311 | 1.00x |
| mbs1 / w0 / **fastloss** | **14499** | **1.06x** |
| mbs1 / w2 / fastloss | 15688 | 0.98x (workers не узкое место) |
| mbs2 / w0 / fastloss | 13740 | 1.06x (peak 8.07 GiB; эфф. батч ×2 → LR/качество перевалидировать) |

Итог: в прод идут `fast_loss_reduction: true` (+6%) поверх `anchor_every: 4` → **суммарно ~1.8–1.95x** к исходному float-туру (28.8 c → ~15.5–16 c/шаг); mbs остаётся 1. Замечено: старт тренинга теперь платит ~2–3 мин за фингерпринт больших файлов (однократно).

### Визуальная валидация рецепта: сетка Turbo-AE4 vs v2 (`output/at_ae4grid/CONTACT_SHEET.png`)

5 колонок (Base / AE4-no-trig / AE4+trig / v2@518 / v2@1250-последняя) × 2 промпта (кровавый канон, карандашный скетч) × 2 сида, 1024², одинаковый сэмплер.

- **Контроль утечки**: AE4-no-trig визуально неотличим от Base во всех клетках — fused-QKV fix + якорь работают как задумано;
- **Кровавый промпт в сетке был бессубъектным** (`crazy smile, blood splatter...` без `1girl`) — он меряет не качество, а **силу prior-сдвига**: ae4 (preserve ~0.010) тянет аниме-персонажа из prior'а адаптера, v2 (preserve 0.0063) остаётся в режиме «base-композиция + стилевая пласт». Это два осознанных режима, а не «провал»: v2 = чистый `base + стиль` (композиция базовая), ae4 = стиль + умеренный prior-shift. Для суждения о плотности стиля промпту нужен явный субъект;
- **Скетчи**: parity — оба несут манеру и персонажа;
- Вердикт: **турбо-рецепт (lr 2e-4, ae4, anchor 8.0) принят как production-стандарт по скорости**; выбор режима — по задаче: нужен строгий `base+стиль` без сдвига композиции → длинный якорь-каждый-шаг (v2-режим), допустим лёгкий prior-shift ради скорости → ae4-рецепт.
